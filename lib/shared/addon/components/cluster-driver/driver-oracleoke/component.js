...
export default Component.extend(ClusterDriver, {
    ...
    if ( !config ) {
      ...
      if (!isEmpty(cloudCredentials)) {
        const singleMatch = cloudCredentials.find((cc) => {
          if (!isEmpty(get(cc, 'ocicredentialConfig'))) {
            return true;
          }
        });

        if (singleMatch) {
          set(config, 'ociCredentialSecret', singleMatch.id);
          set(config, 'fingerprint', singleMatch.ocicredentialConfig.fingerprint);
          set(config, 'tenancyId', singleMatch.ocicredentialConfig.tenancyId);
          set(config, 'userOcid', singleMatch.ocicredentialConfig.userId);
        }
      }
  ...
  actions: {
    ...
    authenticateOCI(cb) {
      let token = get(this, 'cluster.okeEngineConfig.ociCredentialSecret');

      const auth = {
        type: 'cloud',
        token
      };
      const okeVersions = this.oci.request(auth, 'okeVersions', {
        params: {
        }
      });
      ...
      setProperties(this, {
        okeVersions:  this.mapToContent(okeVersions),
        errors:       [],
      });
    ...
    finishAndSelectCloudCredential(cred) {
      if (isEmpty(cred)) {
        set(this, 'config.ociCredentialSecret', null);
        set(this, 'selectedCred', null);
      } else {
        set(this, 'config.ociCredentialSecret', cred.id);
    ...
    loadNodeConfig(cb) {
      ...
      const availabilityDomains = this.oci.request(auth, 'availabilityDomains', {
        params: {
        }
      });
      const nodeShapes = this.oci.request(auth, 'nodeShapes', {
        params: {
        }
      });
      const nodeImages = this.oci.request(auth, 'nodeOkeImages', {
        params: {
        }
      });
      // TODO check for error
      ...
      setProperties(this, {
        availabilityDomais:  this.mapToContent(availabilityDomains),
        nodeShapes:          this.mapToContent(nodeShapes),
        nodeImages:          this.mapToContent(nodeImages),
        errors:       [],
      });
  ...
  regionChoices: computed('model.cloudCredentialId', 'cluster.okeEngineConfig.ociCredentialSecret', async function() {
    let token = get(this, 'cluster.okeEngineConfig.ociCredentialSecret');
  ...
  kubernetesVersionChoices: computed('model.cloudCredentialId', 'cluster.okeEngineConfig.ociCredentialSecret', async function() {
    let token = get(this, 'cluster.okeEngineConfig.ociCredentialSecret');
    ...
    if ( token !== null && token !== '') {
      ...
      const options = await this.oci.request(auth, 'okeVersions');
  ...
  nodeShapeChoices: computed('model.cloudCredentialId', 'cluster.okeEngineConfig.ociCredentialSecret', async function() {
    let token = get(this, 'cluster.okeEngineConfig.ociCredentialSecret');
  ...
  nodeImageChoices: computed('model.cloudCredentialId', 'cluster.okeEngineConfig.ociCredentialSecret', async function() {
    let token = get(this, 'cluster.okeEngineConfig.ociCredentialSecret');
    ...
    if ( token !== null && token !== '') {
      ...
      const options = await this.oci.request(auth, 'nodeOkeImages');
  ...
  canAuthenticate: computed('config.tenancyId', 'config.compartmentId', 'config.userOcid', 'config.fingerprint', 'config.privateKeyContents', function() {
    return get(this, 'config.tenancyId') && get(this, 'config.userOcid') && get(this, 'config.fingerprint') ? false : true;
  }),
  ...
  cloudCredentials: computed('model.cloudCredentials', function() {
    ...
    return cloudCredentials.filter((cc) => !isEmpty(get(cc, 'ocicredentialConfig')));
  }),
